...
xtitle<-expression(paste(italic("r")[alt]," in backcross with ", italic("P. pacificus")))
pdf(file="Fig_ralt_histogram_BC_to_Ppac.pdf",width=4,height=2)
ggplot(sw.data)+geom_histogram(aes(x=SmoothPred))+
  annotate(geom="segment",x=0.1,xend=0.1,y=60000,yend=15000,arrow=arrow(length=unit(10,"pt")))+
  labs(x=xtitle,y="Count")+
  theme(panel.border=element_rect(linetype="solid",fill=NA))
dev.off()

# Determine introgression
sw.data$Int<-ifelse(sw.data$SmoothPred>0.1,1,0)
...
lrt<-glm0$deviance-glm1$deviance
...
write.csv(gt.data,file="Dataset/Genotype_BC1_male_RQTL_1.csv",row.names=F,quote=F)

## Generate cross data

mapthis<-read.cross("csv",file="Dataset/Genotype_BC1_male_RQTL_1.csv",estimate.map=F,crosstype="bc")
...
violin.data.male<-violin.data #Figure is drawn in Fig_coverage_difference.R
